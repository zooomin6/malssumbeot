...
**현재 상태 (2026-07-27, 실제 코드 기준)**
코어 파이프라인 + 채팅 HTTP 계층 + 소셜 로그인/JWT 인증 완성, 단위·통합 테스트 108건.
**모바일 앱 스캐폴딩 착수(`mobile/`, Expo SDK 54)** — 화면은 아직 확인용 1개. 푸시·QA 자동러너 미구현.
**Phase 0 완료(2026-07-15)**: 프롬프트 5건 승인, 위기 sticky 단계 하강(D-020, 이후 D-026로 대체).
**Phase 1 완료(2026-07-16)**: ChatRequest/Response DTO, ChatController, sessionId 바디, 위기 우회불가
= 단일 진입점 보장(인터셉터 후속), `api` 패키지.
**Phase 2 완료(2026-07-20)**: 소셜 로그인(D-022)·`/api/chat` JWT 배선(D-023). 대화 이력 저장은
MVP 범위 밖 결정(D-024).
2026-07-22: 독립 코드 리뷰 후속 — 성경 근거 파이프라인 재설계(D-025), 위기 sticky 1회성 재설계(D-026).
**Phase 3 진행 중**: RN 담당자 = 직접 개발(D-027), Expo 스캐폴딩 완료(2026-07-27, D-028·D-029).
다음은 채팅 UI. MVP 목표는 RN 앱(D-002).
...
- [~] [민규] 카카오/구글/Apple 개발자 콘솔 앱 등록 (클라이언트 ID/시크릿) — 카카오(7/20)·구글 완료.
      ...
      · 카카오: 엠마오 앱 ID `1515709`, 카카오 로그인 활성화, REST API 키 발급. 방식 A(D-022)라
        Client Secret·Redirect URI 불필요. 이메일은 비즈앱 전환 전까지 미수집.
        **플랫폼 키해시(Android)·번들 ID(iOS) 등록은 Phase 3 RN 앱 생성 시 처리**
...
- [x] [민규] RN 담당자 확정 (친구 협업 vs 직접 개발) — 2026-07-23 확정: **직접 개발**(민규, Claude Code와
      함께). UI/UX 실무 경험 없음 → 채팅 UI 라이브러리로 디자인 결정 최소화하는 방향으로 진행(D-027)
- [x] [코드] Expo 프로젝트 스캐폴딩 — 2026-07-27 완료 (`mobile/`, D-028·D-029). expo-router 파일 기반
      라우팅 + Stack 구조, 표시명 `엠마오`, 번들ID·패키지 `com.malssumbeot.emmaus`.
      tsc·expo-doctor(18/18)·안드로이드 번들 export 통과 + **폰 Expo Go 실기동 확인**(D-027 게이트).
      · **SDK 54 고정은 임시**(D-028) — 폰 Expo Go가 SDK 54까지만 지원. 아래 개발 빌드 전환 시 최신으로 올림
- [ ] [코드] 채팅 UI (메시지 리스트 + 성경 구절 인용 블록 구분 렌더링) — `react-native-gifted-chat`,
      구절 블록은 `renderBubble` 커스텀
- [ ] [코드] **개발 빌드 전환** — 카카오 SDK가 네이티브 모듈이라 Expo Go에서 실행 불가. 로그인 착수 시
      EAS Build로 전환 필요(맥 불필요). 이때 Expo SDK도 최신으로 상향(D-028 해제) + 카카오 콘솔에
      안드로이드 플랫폼(패키지명 + 키해시) 등록
...
- [ ] [코드] 로그인 플로우 — ~~대화 이력 동기화~~ D-024(대화 이력 미저장)로 동기화 대상 자체가 없어 제거
      (2026-07-23 정리, 문서 갱신 누락 발견)
...
## Phase 6 — 서버 배포·운영 (출시의 숨은 전제)
> **스토어 출시보다 먼저 되어 있어야 한다.** 지금 백엔드는 로컬(localhost)에서만 돌고,
> DB도 로컬 docker-compose(55432)다. 앱이 스토어에 올라가면 사용자 폰이 공개된 서버를 불러야 하므로
> 배포 없이는 출시 자체가 성립하지 않는다. 2026-07-27 기준 ROADMAP에 통째로 비어 있던 구간을 신설.
> 부수 효과로 백엔드 실무 역량 중 로컬 개발만으로는 배울 수 없는 영역(트래픽·운영)이 여기서 채워진다.

- [ ] [민규] 클라우드 선택 + 계정 (AWS/GCP 중 하나. 외부 결제라 사람 승인 항목)
- [ ] [코드] 백엔드 배포 — 컨테이너 이미지 빌드 + 호스팅. 로컬 docker-compose PostgreSQL →
      관리형 DB(RDS/Cloud SQL)로 이전, Flyway 마이그레이션이 운영 DB에 적용되는 경로 확립
- [ ] [코드] **HTTPS 필수** — 앱↔서버 사이에 JWT와 대화 내용(민감)이 오간다. 평문 전송 불가
- [ ] [코드] 환경별 설정 분리 + **비밀값 관리** — `ANTHROPIC_API_KEY`·JWT 시크릿·카카오 키를
      코드/레포에 두지 않고 주입 (현재는 로컬 env var)
- [ ] [코드] CI/CD — 푸시 시 테스트 자동 실행 + 배포 (현재 테스트 108건을 수동 실행 중)
- [ ] [코드] 로깅·모니터링 — 장애 원인 추적. **위기 응답 경로는 실패하면 안 되는 구간이라
      별도 알림 대상**(단, 위기 대화 내용 자체를 로그에 남기는 범위는 개인정보 검토 필요, D-024 맥락)
- [ ] [코드] `CrisisSessionStore` 인메모리 → Redis 전환 — 서버가 2대 이상이 되는 순간 세션이
      서버별로 갈라져 위기 sticky가 깨진다 (Phase 2에서 "검토"로 미뤄둔 항목의 실제 마감 시점)
- [ ] [검증] Claude API 비용·rate limit 실사용 관측 (현재 앱 rate limit은 시간당 30건)
