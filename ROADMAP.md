...
**현재 상태 (2026-07-28, 실제 코드 기준)**
코어 파이프라인 + 채팅 HTTP 계층 + 소셜 로그인/JWT 인증 완성, 단위·통합 테스트 108건.
**모바일 앱(`mobile/`, Expo SDK 54)** — 스캐폴딩 + 채팅 화면 뼈대(메시지 표시·로컬 전송)까지.
성경 구절 인용 블록 렌더링과 서버 연결은 미완. 푸시·QA 자동러너 미구현.
**설계 원칙을 헌법으로 승격(2026-07-28, D-031)** — 관통 원칙("금지로 막지 말고 검증된 재료를
늘려서 푼다") + 3층 모델(안전/내용/표현). 변경은 사람 승인 항목.
...
- [x] [민규] **상담 프롬프트 개편 승인·반영 — 2026-07-28 완료** (`docs/features/prompt-revision-2026-07.md`).
      [사견 금지]→[출처 구분] 3층위 전환, 말씀 녹임 구조, 원문 무변경 규칙, 값싼 위로·결과 예측·
      번영신학·문맥 무시 인용 금지, 교단 중립 확장, **목회자 비판 금지의 피해 예외 신설**,
      1단계 구절 제안을 위로·규정·서사 3종 5개로 확대. 신학 검사 1차 FAIL(critical) → 지적 6건 중
      5건 수정 후 재검사 통과. 테스트 108건. **정신건강 위기 트리거는 전문가 검토 대상이라 보류**
...
- [~] [코드] 채팅 UI (메시지 리스트 + 성경 구절 인용 블록 구분 렌더링)
      · **완료(2026-07-28)**: `app/chat.tsx` — 말풍선 렌더링 + 입력·로컬 전송. 폰 실기동 확인.
        라이브러리는 `@kesha-antonov/react-native-chat` 4.1.0 — `react-native-gifted-chat`은
        deprecated(유지보수 모드)라 후속 패키지 채택. peer dep `react-native-keyboard-controller`가
        Expo Go에서 동작함을 확인해 개발 빌드 전환은 로그인 단계까지 미룸
      · **남음**: 성경 구절 인용 블록 렌더링. 방식 미정 — A(주소만) / B(주소 + 탭하면 원문 펼침,
        성명표시권 D-016도 여기서 해결) / C(항상 블록). 프롬프트가 출처를 말로 구분하게 됐으므로
        (D-031 3층 모델상 이건 3층 = 사용자 설정 가능 영역) **사용자 설정으로 넘기는 안**이 유력
      · **남음**: 서버 연결. `/api/chat`이 JWT 보호 상태라 로그인 플로우 이후
