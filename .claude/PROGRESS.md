...
1. [~] 채팅 UI — **2026-07-27 진행 중.** 라이브러리는 `@kesha-antonov/react-native-chat` 4.1.0
   (`react-native-gifted-chat`은 deprecated·유지보수 모드라 후속 패키지 채택. D-028로 SDK 상향이
   예정돼 있어 유지보수 끊긴 패키지는 리스크). `app/chat.tsx` — 메시지 표시 + 로컬 전송까지 완료,
   폰 실기동 확인. peer dep `react-native-keyboard-controller`가 Expo Go에서 동작함을 확인해
   개발 빌드 전환은 로그인 단계까지 미룸.
   · **남은 것: 성경 구절 인용 블록 렌더링.** 렌더링 방식은 프롬프트 개편(아래 승인 대기)과
     묶여 있음 — 모델이 출처를 말로 구분하면(출처 구분 원칙) 항상 보이는 블록의 필요성이 줄어듦.
     현재 후보: A(주소만) / B(주소 + 탭하면 원문 펼침, 성명표시권 D-016도 여기서 해결) / C(항상 블록).
     민규 선호는 "말씀을 녹여서 말하기" → B 유력
2. [ ] 로그인 플로우 + 대화 이력 동기화
...
- [ ] 🆕 **프롬프트 개편 변경안 승인 대기 (2026-07-27)** → 전문은 `docs/features/prompt-revision-2026-07.md`
  민규가 실제 상담 케이스("기도해도 응답이 없는데 차라리 무당집 가는 게 낫지 않나")를 외부 모델에
  던져 받은 초안을 우리 제약에 맞게 대조·선별한 결과. **아직 프롬프트 파일에 반영하지 않음.**
