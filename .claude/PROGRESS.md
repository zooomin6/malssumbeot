...
- [ ] 🆕 **로컬 DB `bible_verse` 테이블이 통째로 비어 있음 (2026-08-15 발견)** — 서버 연결
  테스트 중 "출애굽기 2장 1절", "로마서 2장1절" 등 구절 조회가 전부 실패해 확인해보니,
  `bible_verse` 전 도서 verse_count가 0. `flyway_schema_history`는 원본 그대로(2026-06-12·
  2026-07-20 날짜 유지)라 볼륨이 새로 만들어진 건 아니고, 원인 미상으로 데이터만 사라짐.
  **복구 절차**: `BibleTextScraper`(대한성서공회 재스크래핑, 66권 전체·약 40분 소요·외부
  사이트 반복 요청) → TSV 생성 → `BibleTextImporter`(`bible-import` 프로파일)로 재적재.
  크레딧 여유 있을 때 진행. 이 작업 전까지 성경 구절 인용 기능은 항상 "원문 없음" 폴백으로만
...
1. [x] **채팅 UI — 2026-08-03 완료.** 폰 실기동으로 전 항목 확인.
   · **채팅 라이브러리 제거, 직접 구현(D-033)**: `@kesha-antonov/react-native-chat` 걷어내고
     `FlatList`(inverted) + 말풍선 + 입력바로 구성. 화면은 `app/chat.tsx` 한 파일,
     구절 카드만 `components/VerseQuote.tsx`로 분리
   · **구절 인용 = 하이브리드 카드(D-034)**: 카드·출처는 항상 보이고 본문은 두 줄로 자름 →
     탭하면 전문 + 성명표시권 표기 펼침. B안(주소만) 폐기. 성명표시권 문구는 잠정안(D-016 미확정)
   · **키보드 회피 직접 제어(D-035)**: `useReanimatedKeyboardAnimation`으로 높이를 받아
     하단 여백을 만든다. `KeyboardAvoidingView` 미사용 — RN 내장본을 잘못 import한 것이 원인이었음
   · **디자인 v1 반영(D-036)**: 색 팔레트 확정, 커스텀 헤더(제목+부제), "오늘" 날짜 칩,
     테두리형 안내 문구, 빠른 응답 칩, 시안형 입력바
   · **대화 진입 시트 신규**(`app/chat-entry.tsx`): 투명 모달 바텀시트. 엠블럼 + 선택지 3종 +
     "직접 이야기 시작하기". `_layout.tsx`에 `presentation: transparentModal` 등록
...
- [x] ✅ **프롬프트 개편 — 2026-07-28 승인·반영 완료** → 전문은 `docs/features/prompt-revision-2026-07.md`
  민규가 실제 상담 케이스("기도해도 응답이 없는데 차라리 무당집 가는 게 낫지 않나")를 외부 모델에
  던져 받은 초안을 우리 제약에 맞게 대조·선별한 결과. **`master.txt`·`counseling.txt`·
  `verse-address-proposal.txt` 반영 완료. 테스트 108건 통과.**
  · 신학 검사 1차 **FAIL(critical)** → 지적 6건 중 5건 수정 후 재검사에서 닫힘. 수정 내역:
    원칙 4의 "분명히 전하라"를 점술·무속·이단으로 한정하고 정죄 금지 대상(교회 불출석·기도 부족·
    신앙 약화·원망)에는 적용하지 않음을 명시(T2 회귀 차단), 교단 쟁점 열거 한정, 목회자 예외를
    "의심되는 정황 서술"까지 확대 + 교회 밖 창구 안내, few-shot에서 장 단위 인용 제거,
    길이 기준 정합화, 구절 선택 기준(고통 호소 중이면 위로 우선, 단 금하는 행위 시 규정 구절 병기)
  · **잔여 WARN**: counseling 예시의 괄호 지시문이 출력에 섞일 위험 — QA 러너에서 확인 필요
  · **승인 완료(2026-07-27)**: 교단 중립 확장, 출처 구분 3층위(=[사견 금지] 정책 전환),
    값싼 위로·번영신학·결과 예측·문맥 무시 인용 금지, 원망을 죄로 규정하지 않기, few-shot 예시,
    **목회자 비판 금지의 피해 사례 예외 신설**
  · **반영 대기 파일 3종**: `master.txt`(전면), `counseling.txt`(말씀 녹임 구조·길이 예외),
    `verse-address-proposal.txt`(규정·서사 주소까지 제안, 3→5개)
  · 외부 초안에서 **채택하지 않은 것**: 개역개정 지정(절대원칙 6·저작권), 목사 역할 부여(절대원칙 1),
    기억 인용 허용(절대원칙 2), 프롬프트 내 안전 프로토콜(CrisisFilter 구조 유지)
  · **반영 시 필수**: 신학 검사 에이전트 실행(세션 루틴 4) + 테스트 108건 회귀
- [ ] 🆕 **위기 프로토콜 v2 — 등급 분기 도입 (2026-07-28, 승인 + 정신건강 전문가 검토 대기)**
  → `docs/features/crisis-protocol-v2-proposal.md`
  민규 제시 세 발화(①말씀대로 사는 게 어렵다 ②죽고싶다+교회 상처 ③죽고싶다+상사 분노)를 코드로
  추적한 결과 **②와 ③이 완전히 동일한 응답**을 받는다. ③은 분노 토로인데 자살예방 상담전화를 받음.
  · 해법(D-032 원칙, 5-A): 모델을 부르지 않고 **고정 문구를 늘려 조합**한다 —
    `CRISIS_AMBIGUOUS_TEXT`(확인 질문 + 안전 한 줄) 신설, `CHURCH_HURT_BLOCK` 조건부 문단 추가
  · **구현 전 문서 11절(구현 함정 4건) 필독** — 그대로 구현하면 "짜증나" 한마디가 위기로 발동한다
  · 승인 대상: 신규 문구 2종 내용, 보조 패턴 추가, `때려치우` 오탐 수정, B등급 도입 자체
  · 승인 불요(코드): `crisisText()` 분기, `detect()` 다중 카테고리 반환
- [ ] 🆕 **위기 프로토콜 공백 3건 (2026-07-27 발견, 별도 승인 + 전문가 검토)** → 프롬프트 변경안 문서 D절
  1. **교회 내 피해(영적 학대·재정 착취)** — 현 `학대` 패턴은 물리적·성적 피해 중심이라
     "헌금을 계속 요구해요", "목사님이 순종하라고만 하세요"는 안 걸림. 프롬프트 예외 조항이
     1차 방어는 하지만 위기 패턴 추가 여부는 판단 필요
  2. **심각한 정신건강 증상** — 환청을 하나님 음성으로 확신, 조증적 사명감, 현실 감각 상실.
     **패턴에 전혀 없음.** 신앙 앱 특유 위험. T1(계시 '요구')과 달리 사용자의 '확신'이라 성격이 다름.
     정규식으로 잡기 어려워 기존 후속 과제 "애매 신호 LLM 2차 판정"과 묶는 것이 현실적
  3. **이단·사이비 정황** — master 5번·T6에 있으나 위기 레벨 아님. 구체 신호(과도한 헌금 요구,
     외부 관계 차단, 교주 신격화, 시한부 종말) 추가 검토
  · **QA 케이스 T9 신설 제안**: "성경이 명확히 금하는 행위를 사용자가 정당화하며 물어보는 경우"
    유형이 T1~T8에 없음. 핵심 판정 기준 = 바나바가 결과를 예측하면 무당과 같은 자리에 서게 됨
    (삭제·완화가 아닌 추가라 금지 항목은 아니나 승인 대상)
...
- [ ] 성명표시권 표기 문구 확정 (D-016): 개역한글은 재산권은 만료됐지만 성명표시권(저작자 표시)은
  ...
  · **위치는 2026-08-03 해결(D-034)** — 구절 카드를 펼쳤을 때 카드 안에 표시된다. 별도 화면 불필요.
  · **문구는 여전히 미확정.** 현재 코드(`components/VerseQuote.tsx`)의
    `성경전서 개역한글판, 대한성서공회`는 잠정안이며, 확정 시 이 상수만 고치면 된다
- [ ] 🆕 **바나바 브랜드 마크(길의 십자가) PNG 필요** (2026-08-03): 디자인 시안의 아바타·엠블럼에
  쓰이는데 내보낸 파일이 없어 채팅 헤더 아바타와 말풍선 배지를 넣지 못했고, 진입 시트 엠블럼은
  주황 원만 그려둔 상태다. 피그마에서 마크만 PNG로 내보내 `mobile/assets/images/`에 넣으면 반영 가능.
- [ ] 🆕 **대화 진입 시트 선택지를 서버에 넘길지** (2026-08-03, D-036): 시트의 "마음을 나누고
  싶어요/말씀을 찾고 싶어요/기도문이 필요해요"는 사용자가 의도를 미리 고르는 UI인데, 서버는
