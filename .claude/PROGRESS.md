...
> 마지막 갱신: 2026-08-03 (채팅 UI 완성 — 디자인 v1 반영·구절 카드·대화 진입 시트,
> 채팅 라이브러리 제거(D-033)·구절 렌더링 하이브리드(D-034)·키보드 직접 제어(D-035)·
> 디자인 v1(D-036). 백엔드 무변경, 테스트 108건 유지)
...
- [ ] **Phase 3 모바일(RN+Expo) 진행 중 — 2026-07-23 시작.** RN 담당자 = 직접 개발 확정(D-027).
  스캐폴딩(7/27) → **채팅 UI + 대화 진입 시트 완료(8/3, 디자인 v1 반영)**.
  다음 단계 후보: **(a) 서버 연결** — 채팅 화면을 `POST /api/chat`에 붙여 구절 카드에 실제 DB
  원문을 채운다. `/api/chat`이 JWT로 보호돼 있어(D-023) 로그인 또는 개발용 임시 토큰이 선행됨.
  **(b) 로그인 플로우** — 개발 빌드 전환·카카오 키해시 등록 동반. **(c) 홈 화면** — 아래
  "사람 확인 필요"의 구조 결정(탭 전환·저장 기능) 선행 필요.
