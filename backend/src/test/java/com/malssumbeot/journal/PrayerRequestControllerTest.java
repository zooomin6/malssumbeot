...
@WebMvcTest(PrayerRequestController.class)
...
class PrayerRequestControllerTest {
    ...
    void 토큰이_없으면_401이고_서비스를_부르지_않는다() throws Exception {
        mockMvc.perform(post("/api/prayer-requests")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"content\":\"가족을 위해 기도해주세요\"}"))
    ...
    void 생성하면_201과_본문을_반환한다() throws Exception {
        when(service.create(eq(1L), any())).thenReturn(new PrayerRequest(1L, "가족을 위해 기도해주세요"));
        ...
        mockMvc.perform(post("/api/prayer-requests")
                ...
                .andExpect(jsonPath("$.content").value("가족을 위해 기도해주세요"));
    }
    ...
    void 목록_조회는_토큰의_사용자_id로_서비스를_부른다() throws Exception {
        ...
        mockMvc.perform(get("/api/prayer-requests")
    ...
    void 삭제는_204() throws Exception {
        mockMvc.perform(delete("/api/prayer-requests/10")
                        .header(HttpHeaders.AUTHORIZATION, "Bearer " + tokenFor(1L)))
