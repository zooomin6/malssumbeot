...
`VerseReferenceScanner`는 "책 장:절" 패턴만 잡는다. "시편 23편"처럼 장 단위 인용이나,
주소 없이 본문만 풀어쓴 경우는 아직 못 잡는다. RAG(임베딩 검색) 도입 전이라
`<bible_verses>` 블록이 비어 있어 프롬프트 지시에 의존하는 부분도 있다. 이 한계들은
숨기지 않고 PROGRESS.md 다음 작업으로 기록했다. **완벽하지 않은 방어선도, 한계를
아는 채로 쓰면 안전하다. 모르는 채로 쓰는 게 위험하다.**
